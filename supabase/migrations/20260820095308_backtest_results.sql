...
REVOKE ALL ON FUNCTION public.activate_subscription_from_payment(uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.activate_subscription_from_payment(uuid) TO service_role;
...
CREATE FUNCTION public.admin_dismiss_corporate_action_detection (
  p_detection_id uuid,
...
GRANT ALL ON FUNCTION public.admin_dismiss_corporate_action_detection(uuid, text) TO service_role;

CREATE FUNCTION public.admin_grant_premium (
  p_user_id uuid,
...
BEGIN
  ...
  IF p_days IS NULL OR p_days <= 0 THEN
    RAISE EXCEPTION 'p_days harus lebih dari 0';
  END IF;

  -- Kalau user masih punya subscription aktif yang belum habis, perpanjang dari situ.
  -- Kalau enggak, mulai dari sekarang.
  SELECT period_end INTO v_existing_end
  FROM public.subscriptions
  WHERE user_id = p_user_id AND status IN ('active', 'grace')
  ORDER BY period_end DESC NULLS LAST
  LIMIT 1;

  v_new_end := GREATEST(COALESCE(v_existing_end, v_now), v_now) + (p_days || ' days')::interval;

  INSERT INTO public.subscriptions (user_id, status, plan, period_start, period_end, cancel_at_period_end)
  VALUES (p_user_id, 'active', 'premium_monthly_manual', v_now, v_new_end, false);

  UPDATE public.profiles SET is_premium = true WHERE id = p_user_id;

  INSERT INTO public.audit_logs (actor_id, action, entity_type, entity_id, detail)
  VALUES (auth.uid(), 'ADMIN_GRANT_PREMIUM_MANUAL', 'user', p_user_id,
    jsonb_build_object('days', p_days, 'new_period_end', v_new_end));
END;
$function$;

REVOKE ALL ON FUNCTION public.admin_grant_premium(uuid, integer) FROM PUBLIC;

GRANT ALL ON FUNCTION public.admin_grant_premium(uuid, integer) TO authenticated;

GRANT ALL ON FUNCTION public.admin_grant_premium(uuid, integer) TO service_role;

CREATE FUNCTION public.admin_invalidate_signal (
  p_signal_id uuid,
  p_reason    text
)
  RETURNS void
...
REVOKE ALL ON FUNCTION public.admin_invalidate_signal(uuid, text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.admin_invalidate_signal(uuid, text) TO authenticated;

GRANT ALL ON FUNCTION public.admin_invalidate_signal(uuid, text) TO service_role;
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    p.id, u.email, p.full_name, p.is_premium, p.is_admin, p.is_active, p.created_at,
    s.status, s.plan, s.period_end
  FROM public.profiles p
  JOIN auth.users u ON u.id = p.id
  LEFT JOIN LATERAL (
    SELECT status, plan, period_end FROM public.subscriptions
...
GRANT ALL ON FUNCTION public.admin_list_users(integer) TO authenticated;

GRANT ALL ON FUNCTION public.admin_list_users(integer) TO service_role;

CREATE FUNCTION public.admin_promote_corporate_action_detection (
  p_detection_id uuid,
  p_action_type  text,
  p_ex_date      date,
  p_ratio        numeric DEFAULT NULL::numeric,
  p_notes        text    DEFAULT NULL::text
)
  RETURNS uuid
  LANGUAGE plpgsql
...
CREATE FUNCTION public.admin_record_corporate_action (
  p_stock_id    uuid,
  p_action_type text,
  p_ex_date     date,
  p_ratio       numeric DEFAULT NULL::numeric,
...
BEGIN
  ...
  -- FIX: pakai entity_type/entity_id
  INSERT INTO public.audit_logs (actor_id, action, entity_type, entity_id, detail)
  VALUES (
    auth.uid(),
...
GRANT ALL ON FUNCTION public.admin_record_corporate_action(uuid, text, date, numeric, text) TO authenticated;

GRANT ALL ON FUNCTION public.admin_record_corporate_action(uuid, text, date, numeric, text) TO service_role;

CREATE FUNCTION public.admin_revoke_premium (
  p_user_id uuid
)
  RETURNS void
  LANGUAGE plpgsql
...
BEGIN
  ...
  INSERT INTO public.audit_logs (actor_id, action, entity_type, entity_id, detail)
  VALUES (auth.uid(), 'ADMIN_REVOKE_PREMIUM_MANUAL', 'user', p_user_id, '{}'::jsonb);
END;
$function$;

REVOKE ALL ON FUNCTION public.admin_revoke_premium(uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.admin_revoke_premium(uuid) TO authenticated;

GRANT ALL ON FUNCTION public.admin_revoke_premium(uuid) TO service_role;

CREATE FUNCTION public.admin_top_token_users (
...
begin
  ...
  from (
    select
      ...
      p.full_name,
      p.is_premium,
      sum(abs(tt.amount)) as tokens_used
    ...
    where tt.amount < 0
      and tt.created_at >= now() - (greatest(p_days, 1) || ' days')::interval
...
REVOKE ALL ON FUNCTION public.admin_top_token_users(integer) FROM PUBLIC;

GRANT ALL ON FUNCTION public.admin_top_token_users(integer) TO authenticated;
...
CREATE FUNCTION public.assert_signal_candles_fresh()
  ...
  SECURITY DEFINER
  SET search_path TO 'public'
  AS $function$ declare stale_count integer; begin select count(*) into stale_count from (select distinct on (c.timeframe) c.timeframe,c.ts from public.candles c where c.stock_id=new.stock_id and c.timeframe in (coalesce(new.entry_timeframe,new.timeframe), new.bias_timeframe) order by c.timeframe,c.ts desc) x where x.ts < now()-interval '7 days'; if stale_count > 0 then raise exception 'SIGNAL_BLOCKED_STALE_CANDLES: stock_id=% entry_tf=% bias_tf=%',new.stock_id,new.entry_timeframe,new.bias_timeframe using errcode='P0001'; end if; return new; end; $function$;
...
CREATE FUNCTION public.classify_corporate_action_text (
  ...
  AS $function$
  select case
    when p_text ~* '(reverse\s*(stock\s*)?split|penggabungan\s*saham|reverse\s*stock)' then 'REVERSE_SPLIT'
    when p_text ~* '(stock\s*split|pemecahan\s*(nilai\s*)?saham|split\s*saham)' then 'STOCK_SPLIT'
    when p_text ~* '(rights?\s*issue|hmetd|penawaran\s*umum\s*terbatas|puutd?)' then 'RIGHTS_ISSUE'
    when p_text ~* '(saham\s*bonus|bonus\s*share)' then 'BONUS_SHARES'
    when p_text ~* '(waran\s*(seri)?|warrant)' then 'WARRANT'
    when p_text ~* '(dividen|dividend)' then 'DIVIDEND'
    when p_text ~* '(delisting|dikeluarkan\s*dari\s*(pencatatan|bursa))' then 'DELISTING'
    when p_text ~* '(suspensi|suspend(ed)?|penghentian\s*sementara\s*perdagangan)' then 'SUSPENSION'
    else null
  end;
$function$;
...
CREATE FUNCTION public.credit_ad_unlock_verified (
  p_user_id        uuid,
  p_stock_id       uuid,
  p_transaction_id text,
  p_reward_item    text    DEFAULT NULL::text,
...
REVOKE ALL ON FUNCTION public.credit_ad_unlock_verified(uuid, uuid, text, text, numeric) FROM PUBLIC;

GRANT ALL ON FUNCTION public.credit_ad_unlock_verified(uuid, uuid, text, text, numeric) TO service_role;

CREATE FUNCTION public.deduct_token (
  p_type         text,
...
REVOKE ALL ON FUNCTION public.deduct_token(text, uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.deduct_token(text, uuid) TO authenticated;

GRANT ALL ON FUNCTION public.deduct_token(text, uuid) TO service_role;

CREATE FUNCTION public.detect_unusual_activity()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.dispatch_morning_briefing()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.enforce_ai_task_limits_on_update()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
CREATE FUNCTION public.enforce_ai_task_limits()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
CREATE FUNCTION public.enforce_trading_plan_module_lock_insert()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.enforce_trading_plan_module_lock()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.enforce_trading_plan_tier()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.enforce_watchlist_folder_limit()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.enforce_watchlist_item_limit()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.execute_ai_tasks()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.expire_active_subscriptions()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.expire_grace_subscriptions()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
GRANT ALL ON FUNCTION public.get_active_signal_map() TO authenticated;

GRANT ALL ON FUNCTION public.get_active_signal_map() TO service_role;

CREATE FUNCTION public.get_for_you_stocks (
...
REVOKE ALL ON FUNCTION public.get_for_you_stocks(integer) FROM PUBLIC;

GRANT ALL ON FUNCTION public.get_for_you_stocks(integer) TO authenticated;

GRANT ALL ON FUNCTION public.get_for_you_stocks(integer) TO service_role;

CREATE FUNCTION public.get_full_signal_map (
...
CREATE FUNCTION public.get_next_trading_day (
  from_date          date,
  trading_days_ahead integer DEFAULT 1
)
  RETURNS date
  LANGUAGE plpgsql
  STABLE
...
declare
  ...
  remaining integer := trading_days_ahead;
begin
  while remaining > 0 loop
    cursor_date := cursor_date + interval '1 day';
    if public.is_trading_day(cursor_date) then
...
GRANT ALL ON FUNCTION public.get_next_trading_day(date, integer) TO anon;

GRANT ALL ON FUNCTION public.get_next_trading_day(date, integer) TO authenticated;

GRANT ALL ON FUNCTION public.get_next_trading_day(date, integer) TO service_role;

CREATE FUNCTION public.get_signal_for_stock (
  p_stock_id uuid,
  p_tier     text DEFAULT 'daily'::text
)
  RETURNS jsonb
...
GRANT ALL ON FUNCTION public.get_signal_for_stock(uuid, text) TO anon;

GRANT ALL ON FUNCTION public.get_signal_for_stock(uuid, text) TO authenticated;

GRANT ALL ON FUNCTION public.get_signal_for_stock(uuid, text) TO service_role;
...
GRANT ALL ON FUNCTION public.get_signal_history(text, text, integer, integer, integer) TO anon;

GRANT ALL ON FUNCTION public.get_signal_history(text, text, integer, integer, integer) TO authenticated;
...
GRANT ALL ON FUNCTION public.get_watch_state_for_stock(uuid, text) TO anon;

GRANT ALL ON FUNCTION public.get_watch_state_for_stock(uuid, text) TO authenticated;

GRANT ALL ON FUNCTION public.get_watch_state_for_stock(uuid, text) TO service_role;
...
CREATE FUNCTION public.is_trading_day (
  ...
  AS $function$
  select coalesce(
    (select is_trading_day from market_calendar where date = check_date),
    -- fallback kalau tanggal di luar range yang sudah di-backfill: anggap weekday = trading day
    extract(dow from check_date) not in (0,6)
  );
$function$;

GRANT ALL ON FUNCTION public.is_trading_day(date) TO anon;

GRANT ALL ON FUNCTION public.is_trading_day(date) TO authenticated;

GRANT ALL ON FUNCTION public.is_trading_day(date) TO service_role;

CREATE FUNCTION public.job_run_finish (
  p_id     uuid,
  p_status text,
...
REVOKE ALL ON FUNCTION public.job_run_finish(uuid, text, jsonb) FROM PUBLIC;

GRANT ALL ON FUNCTION public.job_run_finish(uuid, text, jsonb) TO service_role;

CREATE FUNCTION public.job_run_start (
  p_job_name text,
...
REVOKE ALL ON FUNCTION public.job_run_start(text, jsonb) FROM PUBLIC;

GRANT ALL ON FUNCTION public.job_run_start(text, jsonb) TO service_role;
...
GRANT ALL ON FUNCTION public.list_active_signals(text) TO anon;
  ...
  WHILE clock_timestamp() < v_deadline AND EXISTS (SELECT 1 FROM public.mtf_pipeline_runs WHERE status='RUNNING') LOOP
    FOR r IN SELECT * FROM public.mtf_pipeline_runs WHERE status='RUNNING' ORDER BY created_at LOOP
      ...
      IF v_row.error_msg IS NOT NULL OR v_row.status_code IS NULL OR v_row.status_code < 200 OR v_row.status_code >= 300 THEN
        -- Transient error handling: retry stage yang sama (bukan langsung ERROR),
        -- sampai MAX_RETRIES kali. Ini nangkep kasus kayak "JWT issued at future"
        -- yang biasanya hilang sendiri kalau dicoba ulang sebentar lagi.
        IF r.retry_count < MAX_RETRIES THEN
          v_retry_url := CASE r.stage
            WHEN 'fetch-candles' THEN v_url || '/functions/v1/fetch-candles?timeframe=' || (r.timeframes[r.tf_index]) || '&offset=' || r.offset_val || '&limit=' || PAGE_SIZE
            WHEN 'compute-indicators' THEN v_url || '/functions/v1/compute-indicators?timeframe=' || (r.timeframes[r.tf_index]) || '&offset=' || r.offset_val || '&limit=' || PAGE_SIZE
            WHEN 'generate-signals-mtf' THEN v_url || '/functions/v1/generate-signals-mtf?tier=' || r.tier || '&offset=' || r.offset_val || '&limit=' || PAGE_SIZE
            ELSE NULL
          END;

          IF v_retry_url IS NOT NULL THEN
            PERFORM pg_sleep(4);
            SELECT net.http_post(
              url := v_retry_url,
              headers := v_headers, timeout_milliseconds := 240000
            ) INTO v_req;
            UPDATE public.mtf_pipeline_runs
              SET pending_request_id = v_req, retry_count = r.retry_count + 1, updated_at = now(),
                  detail = r.detail || jsonb_build_object('last_retry_reason', jsonb_build_object('status_code', v_row.status_code, 'error_msg', v_row.error_msg, 'content', left(v_row.content,300)))
              WHERE id = r.id;
            CONTINUE;
          END IF;
        END IF;

        PERFORM public.job_run_finish(r.job_run_id, 'ERROR', jsonb_build_object('stage', r.stage, 'offset', r.offset_val, 'status_code', v_row.status_code, 'error_msg', v_row.error_msg, 'content', left(v_row.content,500), 'retries_used', r.retry_count));
        UPDATE public.mtf_pipeline_runs SET status='ERROR', updated_at=now() WHERE id = r.id;
        CONTINUE;
      END IF;
      ...
      IF r.stage IN ('fetch-candles','compute-indicators') AND v_page_total >= PAGE_SIZE THEN
        ...
        UPDATE public.mtf_pipeline_runs SET offset_val=v_next_offset, pending_request_id=v_req, retry_count=0, updated_at=now() WHERE id = r.id;
      ...
      ELSIF r.stage = 'fetch-candles' THEN
        ...
        UPDATE public.mtf_pipeline_runs SET stage='compute-indicators', offset_val=0, pending_request_id=v_req, retry_count=0, updated_at=now() WHERE id = r.id;
      ...
      ELSIF r.stage = 'compute-indicators' THEN
        IF r.tf_index < array_length(r.timeframes, 1) THEN
          ...
          UPDATE public.mtf_pipeline_runs SET stage='fetch-candles', tf_index = r.tf_index + 1, offset_val=0, pending_request_id=v_req, retry_count=0, updated_at=now() WHERE id = r.id;
        ELSE
          SELECT net.http_post(
            url := v_url || '/functions/v1/generate-signals-mtf?tier=' || r.tier || '&offset=0&limit=' || PAGE_SIZE,
            headers := v_headers, timeout_milliseconds := 240000
          ) INTO v_req;
          UPDATE public.mtf_pipeline_runs SET stage='generate-signals-mtf', offset_val=0, pending_request_id=v_req, retry_count=0, updated_at=now() WHERE id = r.id;
        END IF;
...
CREATE FUNCTION public.notify_push_on_new_notification()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
...
CREATE FUNCTION public.notify_subscription_renewal_h1()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.notify_unusual_activity_subscribers()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.notify_watchlist_new_signal()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.notify_watchlist_news()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.process_corporate_actions()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.process_pending_corporate_actions()
  RETURNS jsonb
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: pakai entity_type/entity_id (bukan target_table/target_id yang tidak ada)
    INSERT INTO public.audit_logs (actor_id, action, entity_type, entity_id, detail)
    VALUES (
      NULL,
      ...
      v_action.id,
      jsonb_build_object(
        'stock_id', v_action.stock_id,
        'action_type', v_action.action_type,
        'signals_invalidated', v_count
      )
    );
  ...
  RETURN jsonb_build_object(
    ...
    'signals_invalidated', v_signals_invalidated
  );
END;
$function$;
...
CREATE FUNCTION public.protect_idx_manual_candle()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SET search_path TO 'public'
  AS $function$
BEGIN
  -- IDX_MANUAL tidak boleh ditimpa oleh source apapun pada hari yang sama.
  IF OLD.source = 'IDX_MANUAL'
     AND NEW.source IS DISTINCT FROM 'IDX_MANUAL'
     AND (OLD.ts AT TIME ZONE 'Asia/Jakarta')::date
       = (COALESCE(NEW.ts, OLD.ts) AT TIME ZONE 'Asia/Jakarta')::date
  THEN
    RETURN OLD;
  END IF;

  -- IDX_AUTO tidak boleh ditimpa oleh YAHOO pada hari yang sama.
  -- (IDX_AUTO tetap boleh ditimpa oleh IDX_MANUAL atau IDX_AUTO berikutnya -- update
  -- data lebih baru dari sumber sederajat/lebih otoritatif tetap diizinkan.)
  IF OLD.source = 'IDX_AUTO'
     AND NEW.source = 'YAHOO'
     AND (OLD.ts AT TIME ZONE 'Asia/Jakarta')::date
       = (COALESCE(NEW.ts, OLD.ts) AT TIME ZONE 'Asia/Jakarta')::date
  THEN
    RETURN OLD;
...
COMMENT ON FUNCTION public.protect_idx_manual_candle() IS 'Proteksi candle dari overwrite silang sumber data pada hari yang sama: IDX_MANUAL > IDX_AUTO > YAHOO. Diperluas 20 Agustus 2026 untuk cover kasus IDX_AUTO vs YAHOO (root cause data campur).';
...
CREATE FUNCTION public.protect_idx_manual_quote()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.protect_sensitive_profile_columns()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.protect_signal_immutable_fields()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.push_market_cap_to_quote()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.quote_is_fresh (
  p_market_time timestamp with time zone,
  ...
  IMMUTABLE
  SET search_path TO ''
  AS $function$ select p_market_time is not null and p_market_time >= p_now - interval '7 days' $function$;

GRANT ALL ON FUNCTION public.quote_is_fresh(timestamp WITH time zone, timestamp WITH time zone) TO authenticated;
...
BEGIN
  ...
  -- FIX (audit 19 Agustus 2026 lanjutan): signal-pipeline-mtf-swing (D1+W1, ~2x
  -- lebih banyak tahapan paginated dibanding daily) kadang belum selesai di menit
  -- ke-15 meski mtf_pipeline_poll masih aktif progress tiap menit (apalagi kalau
  -- ada retry karena error transient "JWT issued at future"). Kasih longgar 25 menit
  -- khusus job swing, job lain tetap 15 menit seperti semula.
  UPDATE public.job_runs
  SET status = 'ERROR', finished_at = now(),
      detail = COALESCE(detail, '{}'::jsonb) || jsonb_build_object('reason', 'timeout/no response after 15 min')
  WHERE status = 'RUNNING'
    AND job_name NOT LIKE 'signal-pipeline-mtf-swing%'
    AND started_at < now() - interval '15 minutes';

  UPDATE public.job_runs
  SET status = 'ERROR', finished_at = now(),
      detail = COALESCE(detail, '{}'::jsonb) || jsonb_build_object('reason', 'timeout/no response after 25 min (swing pipeline)')
  WHERE status = 'RUNNING'
    AND job_name LIKE 'signal-pipeline-mtf-swing%'
...
CREATE FUNCTION public.record_signal_result()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
CREATE FUNCTION public.refund_token (
  p_type         text,
  p_reference_id uuid DEFAULT NULL::uuid
)
  RETURNS TABLE (
...
BEGIN
  ...
  SELECT tw.id, tw.balance INTO v_wallet_id, v_balance
  FROM public.token_wallets tw
  WHERE tw.user_id = v_user_id
  FOR UPDATE;
  ...
  -- FIX (19 Agustus 2026): predicate ON CONFLICT disamakan persis dengan
  -- token_transactions_credit_type_reference_id_uniq (reference_id IS NOT NULL
  -- AND amount > 0) supaya Postgres bisa infer index arbiter-nya. Predicate lama
  -- "WHERE reference_id IS NOT NULL" saja tidak match index manapun di tabel ini,
  -- jadi setiap panggilan refund_token() SELALU error 42P10 (no unique or
  -- exclusion constraint matching ON CONFLICT), dan token yang kepotong pas AI
  -- gagal tidak pernah balik ke user.
  INSERT INTO public.token_transactions (wallet_id, amount, type, reference_id, balance_before, balance_after)
  VALUES (v_wallet_id, 1, p_type, p_reference_id, v_balance, v_balance + 1)
  ON CONFLICT (type, reference_id) WHERE reference_id IS NOT NULL AND amount > 0 DO NOTHING
...
REVOKE ALL ON FUNCTION public.refund_token(text, uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.refund_token(text, uuid) TO authenticated;

GRANT ALL ON FUNCTION public.refund_token(text, uuid) TO service_role;
...
GRANT ALL ON FUNCTION public.request_account_deletion(text) TO authenticated;

GRANT ALL ON FUNCTION public.request_account_deletion(text) TO service_role;

CREATE FUNCTION public.retry_signal_pipeline_mtf_if_needed (
  p_tier text
)
  RETURNS void
  LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT count(*) INTO v_running FROM public.mtf_pipeline_runs
    WHERE tier = p_tier AND status = 'RUNNING';
  IF v_running > 0 THEN
    RETURN; -- masih jalan (baik run pertama atau hasil retry sebelumnya), jangan diganggu
  END IF;

  SELECT count(*) INTO v_success_today FROM public.mtf_pipeline_runs
    WHERE tier = p_tier AND status = 'SUCCESS'
      AND (created_at AT TIME ZONE 'Asia/Jakarta')::date = v_today;
  IF v_success_today > 0 THEN
    RETURN; -- sudah ada run sukses hari ini, tidak perlu retry
  END IF;
...
REVOKE ALL ON FUNCTION public.retry_signal_pipeline_mtf_if_needed(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.retry_signal_pipeline_mtf_if_needed(text) TO service_role;
...
CREATE FUNCTION public.run_backtest (
  p_timeframe    text,
...
CREATE FUNCTION public.sync_corporate_action_detections()
  RETURNS jsonb
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
begin
  for v_row in
    select ne.id as news_event_id, ne.stock_id, ne.evidence_summary,
           coalesce(n.title,'') as title, coalesce(n.summary,'') as summary
    from public.news_events ne
    join public.news n on n.id = ne.primary_article_id
    where ne.event_type = 'AKSI_KORPORASI'
      and ne.stock_id is not null
      and not exists (
        select 1 from public.corporate_action_detections cad
        where cad.news_event_id = ne.id
      )
  loop
    v_combined_text := coalesce(v_row.title,'') || ' ' || coalesce(v_row.summary,'') || ' ' || coalesce(v_row.evidence_summary,'');
    v_action_type := public.classify_corporate_action_text(v_combined_text);

    insert into public.corporate_action_detections
      (stock_id, news_event_id, detected_action_type, evidence_summary)
    values
      (v_row.stock_id, v_row.news_event_id, v_action_type, v_row.evidence_summary);

    v_inserted := v_inserted + 1;
  end loop;

  return jsonb_build_object('detections_inserted', v_inserted);
end;
$function$;

REVOKE ALL ON FUNCTION public.sync_corporate_action_detections() FROM PUBLIC;

GRANT ALL ON FUNCTION public.sync_corporate_action_detections() TO service_role;

CREATE FUNCTION public.sync_quote_market_cap()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.sync_signal_evidence()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
REVOKE ALL ON FUNCTION public.sync_signal_evidence() FROM PUBLIC;

GRANT ALL ON FUNCTION public.sync_signal_evidence() TO service_role;

CREATE FUNCTION public.sync_stock_master()
  RETURNS jsonb
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.trigger_ai_task_executor()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
CREATE FUNCTION public.trigger_detect_unusual_activity()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
CREATE FUNCTION public.trigger_evaluate_session2_preview()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
DECLARE
  ...
  v_req_id bigint;
BEGIN
  v_run_id := public.job_run_start('evaluate-session2-preview');
  ...
  SELECT net.http_post(
    url := v_url || '/functions/v1/evaluate-session2-preview',
    headers := jsonb_build_object('Authorization', 'Bearer ' || v_key, 'Content-Type', 'application/json', 'x-worker-secret', v_secret),
...
REVOKE ALL ON FUNCTION public.trigger_evaluate_session2_preview() FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_evaluate_session2_preview() TO service_role;

CREATE FUNCTION public.trigger_evaluate_signals()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
CREATE FUNCTION public.trigger_fetch_earnings_calendar()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public', 'extensions'
...
CREATE FUNCTION public.trigger_fetch_economic_calendar()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public', 'extensions'
...
CREATE FUNCTION public.trigger_fetch_fundamentals()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public', 'extensions'
...
CREATE FUNCTION public.trigger_fetch_idx_eod()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
BEGIN
  v_run_id := public.job_run_start('fetch-idx-eod');
  SELECT decrypted_secret INTO v_url FROM vault.decrypted_secrets WHERE name = 'project_url';
  SELECT decrypted_secret INTO v_key FROM vault.decrypted_secrets WHERE name = 'service_role_key';
  SELECT value INTO v_secret FROM public.internal_secrets WHERE key = 'worker_shared_secret';
  IF v_url IS NULL OR v_key IS NULL OR v_secret IS NULL THEN
  ...
  END IF;
  SELECT net.http_post(
    url := v_url || '/functions/v1/fetch-idx-eod',
    headers := jsonb_build_object('Authorization','Bearer ' || v_key,'Content-Type','application/json','x-worker-secret', v_secret),
...
REVOKE ALL ON FUNCTION public.trigger_fetch_idx_eod() FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_fetch_idx_eod() TO service_role;

CREATE FUNCTION public.trigger_fetch_index()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
CREATE FUNCTION public.trigger_fetch_news (
...
REVOKE ALL ON FUNCTION public.trigger_fetch_news(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_fetch_news(text) TO service_role;

CREATE FUNCTION public.trigger_fetch_quotes()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
CREATE FUNCTION public.trigger_generate_trending_reason()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO '', 'extensions'
...
BEGIN
  ...
  IF v_url IS NULL OR v_key IS NULL OR v_secret IS NULL THEN
    ...
    RETURN;
  END IF;
  -- Audit 19 Agustus 2026 (lanjutan): limit=40 + concurrency paralel di edge function ternyata
  -- masih sering timeout karena 9Router mode 'auto' butuh sampai ~45 detik/request buat nemu
  -- model yang jalan. Turunkan limit ke 12/panggilan (masih tiap 15 menit = ~1150/hari, jauh di atas
  -- backlog 909) dan naikkan pg_net timeout ke 150 detik biar nggak keburu putus.
  SELECT net.http_post(
    url := v_url || '/functions/v1/generate-trending-reason?limit=12',
    headers := jsonb_build_object('Authorization','Bearer ' || v_key,'Content-Type','application/json','x-worker-secret', v_secret),
    timeout_milliseconds := 150000
  ) INTO v_req_id;
  PERFORM public.job_run_finish(v_run_id, 'SUCCESS', jsonb_build_object('net_request_id', v_req_id, 'note', 'dispatched, menunggu reconcile'));
...
CREATE FUNCTION public.trigger_master_sync_stock()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.trigger_morning_briefing()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
CREATE FUNCTION public.trigger_process_corporate_actions()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public', 'pg_temp'
...
CREATE FUNCTION public.trigger_signal_pipeline_mtf (
  p_tier text
)
  RETURNS void
  LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION public.trigger_signal_pipeline_mtf(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_signal_pipeline_mtf(text) TO service_role;

CREATE FUNCTION public.trigger_signal_regen_if_yahoo (
  p_tier text
)
  RETURNS void
  LANGUAGE plpgsql
...
DECLARE
  ...
  v_yahoo_count int;
  v_url text; v_key text; v_secret text; v_headers jsonb;
  v_req bigint; v_run_id uuid; v_timeframes text[];
BEGIN
  IF p_tier NOT IN ('daily','swing') THEN
    RETURN;
  END IF;
  ...
  SELECT count(*) INTO v_running FROM public.mtf_pipeline_runs
    WHERE tier = p_tier AND status = 'RUNNING';
  IF v_running > 0 THEN
    RETURN; -- ada run lain lagi jalan, jangan tabrakan
  END IF;

  -- Cuma regen kalau ada signal ACTIVE hari ini untuk tier ini yang masih
  -- berbasis YAHOO_FALLBACK (spec 22.2/section 3.2 -- signals.data_source).
  SELECT count(*) INTO v_yahoo_count
  FROM public.signals
  WHERE signal_tier = p_tier
    AND status = 'ACTIVE'
    AND data_source = 'YAHOO_FALLBACK'
    AND (triggered_at AT TIME ZONE 'Asia/Jakarta')::date = v_today;

  IF v_yahoo_count = 0 THEN
    RETURN; -- tidak ada yang perlu di-regen (belum ada signal sama sekali hari ini,
             -- atau signal hari ini memang udah dari IDX)
  END IF;

  v_timeframes := CASE WHEN p_tier = 'daily' THEN ARRAY['D1'] ELSE ARRAY['D1','W1'] END;
  v_run_id := public.job_run_start('signal-regen-idx-' || p_tier);
  ...
  -- Skip fetch-candles D1 (sudah fresh IDX_AUTO) -> langsung compute-indicators.
  -- Poller (mtf_pipeline_poll) yang sudah ada akan otomatis lanjut ke fetch-candles
  -- W1 (untuk swing) lalu generate-signals-mtf, sesuai state machine yang sudah ada.
  SELECT net.http_post(
    url := v_url || '/functions/v1/compute-indicators?timeframe=D1&offset=0&limit=100',
    headers := v_headers, timeout_milliseconds := 240000
  ) INTO v_req;

  INSERT INTO public.mtf_pipeline_runs(tier, status, stage, timeframes, tf_index, offset_val, pending_request_id, job_run_id, detail)
  VALUES (p_tier, 'RUNNING', 'compute-indicators', v_timeframes, 1, 0, v_req, v_run_id,
    jsonb_build_object('reason', 'regenerate signal pakai data IDX setelah sebelumnya pakai Yahoo fallback', 'yahoo_signal_count', v_yahoo_count));
END;
$function$;

COMMENT ON FUNCTION public.trigger_signal_regen_if_yahoo(text) IS 'Regenerasi signal harian otomatis begitu IDX EOD berhasil, kalau signal hari itu sebelumnya sempat dibuat pakai Yahoo fallback. Dipanggil dari fetch-idx-eod setelah sukses. Opsi B (20 Agustus 2026).';

REVOKE ALL ON FUNCTION public.trigger_signal_regen_if_yahoo(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_signal_regen_if_yahoo(text) TO service_role;

CREATE FUNCTION public.trigger_sync_corporate_action_detections()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO 'public'
...
declare
  ...
  v_result jsonb;
begin
  v_run_id := public.job_run_start('sync-corporate-action-detections');
  begin
    v_result := public.sync_corporate_action_detections();
    perform public.job_run_finish(v_run_id, 'SUCCESS', v_result);
  exception when others then
    perform public.job_run_finish(v_run_id, 'ERROR', jsonb_build_object('error', sqlerrm));
  end;
...
REVOKE ALL ON FUNCTION public.trigger_sync_corporate_action_detections() FROM PUBLIC;

GRANT ALL ON FUNCTION public.trigger_sync_corporate_action_detections() TO service_role;

CREATE FUNCTION public.trigger_trending_score()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
CREATE FUNCTION public.trigger_unusual_activity_detection()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
CREATE FUNCTION public.unlock_signal_with_ad (
  p_stock_id uuid
)
  RETURNS jsonb
  LANGUAGE plpgsql
...
GRANT ALL ON FUNCTION public.unlock_signal_with_ad(uuid) TO service_role;

CREATE FUNCTION public.unlock_signal_with_token (
  p_stock_id        uuid,
...
REVOKE ALL ON FUNCTION public.unlock_signal_with_token(uuid, uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.unlock_signal_with_token(uuid, uuid) TO authenticated;

GRANT ALL ON FUNCTION public.unlock_signal_with_token(uuid, uuid) TO service_role;

CREATE FUNCTION public.upsert_signal_watch_state (
  p_stock_id         uuid,
  p_signal_tier      text,
  p_state            text,
  p_bias             text,
  p_watch_direction  text,
  p_watch_zone_low   numeric,
  p_watch_zone_high  numeric,
  p_watch_zone_type  text,
  p_trigger_state    text,
  p_reason           text,
  p_support_level    numeric,
  p_resistance_level numeric,
  ...
  AS $function$
begin
  insert into public.signal_watch_states as w (
    stock_id, signal_tier, state, bias, watch_direction,
    watch_zone_low, watch_zone_high, watch_zone_type, trigger_state, reason,
    support_level, resistance_level, last_close, formula_version, data_source,
    first_seen_at, updated_at
  ) values (
    p_stock_id, p_signal_tier, p_state, p_bias, p_watch_direction,
    p_watch_zone_low, p_watch_zone_high, p_watch_zone_type, p_trigger_state, p_reason,
    p_support_level, p_resistance_level, p_last_close, p_formula_version, p_data_source,
    now(), now()
  )
  on conflict (stock_id, signal_tier) do update set
    state = excluded.state,
    bias = excluded.bias,
    watch_direction = excluded.watch_direction,
    watch_zone_low = excluded.watch_zone_low,
    watch_zone_high = excluded.watch_zone_high,
    watch_zone_type = excluded.watch_zone_type,
    ...
    updated_at = now(),
    first_seen_at = case when w.state = excluded.state then w.first_seen_at else now() end;
end;
$function$;
...
CREATE FUNCTION public.wait_for_net_response (
  p_request_id       bigint,
  p_max_wait_seconds integer DEFAULT 100
)
  RETURNS jsonb
  LANGUAGE plpgsql
  SET search_path TO 'public', 'pg_temp'
...
GRANT ALL ON FUNCTION public.wait_for_net_response(bigint, integer) TO anon;

GRANT ALL ON FUNCTION public.wait_for_net_response(bigint, integer) TO authenticated;

GRANT ALL ON FUNCTION public.wait_for_net_response(bigint, integer) TO service_role;
...
ALTER TABLE public.ad_verifications
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.ad_verifications
  ADD CONSTRAINT ad_verifications_pkey PRIMARY KEY (transaction_id);

GRANT ALL ON public.ad_verifications TO authenticated;

GRANT ALL ON public.ad_verifications TO service_role;

CREATE INDEX idx_ad_verifications_user_id ON public.ad_verifications (user_id);

CREATE INDEX idx_ad_verifications_stock_id ON public.ad_verifications (stock_id);
...
GRANT ALL ON public.agreement_acceptances TO anon;

GRANT ALL ON public.agreement_acceptances TO authenticated;

GRANT ALL ON public.agreement_acceptances TO service_role;

CREATE INDEX idx_agreement_acceptances_agreement_id ON public.agreement_acceptances (agreement_id);

CREATE POLICY "own agreement acceptance insert" ON public.agreement_acceptances
  FOR INSERT
  WITH CHECK ((( SELECT auth.uid() AS uid) = user_id));

CREATE POLICY "own agreement acceptance select" ON public.agreement_acceptances
  FOR SELECT
...
CREATE POLICY "agreements readable by all" ON public.agreements
  FOR SELECT
  USING (true);
...
ALTER TABLE public.ai_messages
  ADD CONSTRAINT ai_messages_role_check CHECK (role = ANY (ARRAY['user'::text, 'assistant'::text]));
...
ALTER TABLE public.ai_tasks
  ADD CONSTRAINT ai_tasks_action_check
    CHECK
    (action IS NULL OR (action = ANY (ARRAY['READ_MARKET_DATA'::text, 'READ_WATCHLIST'::text, 'READ_SIGNALS'::text, 'READ_NEWS'::text, 'CREATE_TRADING_PLAN'::text,
    'UPDATE_WATCHLIST'::text, 'CREATE_ALERT'::text, 'SEND_NOTIFICATION'::text])));

ALTER TABLE public.ai_tasks
  ADD CONSTRAINT ai_tasks_permission_decision_check CHECK (permission_decision = ANY (ARRAY['ALLOW'::text, 'DENY'::text, 'PENDING'::text]));

ALTER TABLE public.ai_tasks
  ADD CONSTRAINT ai_tasks_pkey PRIMARY KEY (id);

ALTER TABLE public.ai_tasks
  ADD CONSTRAINT ai_tasks_status_check CHECK (status = ANY (ARRAY['ACTIVE'::text, 'PAUSED'::text, 'DONE'::text, 'FAILED'::text]));

ALTER TABLE public.ai_tasks
  ADD CONSTRAINT ai_tasks_task_type_check CHECK (task_type = ANY (ARRAY['PRICE_ALERT'::text, 'LEVEL_RETEST'::text, 'DAILY_SUMMARY'::text, 'UNUSUAL_VOLUME'::text]));

GRANT ALL ON public.ai_tasks TO anon;

GRANT ALL ON public.ai_tasks TO authenticated;

GRANT ALL ON public.ai_tasks TO service_role;

CREATE INDEX idx_ai_tasks_audit_id ON public.ai_tasks (audit_id);
...
CREATE TRIGGER ai_tasks_enforce_limits
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_ai_task_limits();

CREATE TRIGGER ai_tasks_enforce_limits_update
  BEFORE UPDATE ON public.ai_tasks
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_ai_task_limits_on_update();

CREATE POLICY "own ai tasks" ON public.ai_tasks
  USING ((( SELECT auth.uid() AS uid) = user_id));
...
GRANT ALL ON public.ai_threads TO anon;
...
CREATE INDEX idx_ai_usage_thread_id ON public.ai_usage (thread_id);
...
ALTER TABLE public.app_ratings
  ADD CONSTRAINT app_ratings_rating_check CHECK (rating >= 1 AND rating <= 5);

GRANT ALL ON public.app_ratings TO authenticated;
...
CREATE POLICY "own rating update" ON public.app_ratings
  FOR UPDATE
  TO authenticated
  USING ((( SELECT auth.uid() AS uid) = user_id))
  WITH CHECK ((( SELECT auth.uid() AS uid) = user_id));
...
CREATE TABLE public.backtest_gate_config (
...
ALTER TABLE public.backtest_runs
  ADD CONSTRAINT backtest_runs_level_mode_check CHECK (level_mode IS NULL OR (level_mode = ANY (ARRAY['SCALP'::text, 'STANDARD'::text, 'SWING'::text])));

ALTER TABLE public.backtest_runs
  ADD CONSTRAINT backtest_runs_pkey PRIMARY KEY (id);

ALTER TABLE public.backtest_runs
  ADD CONSTRAINT backtest_runs_run_by_fkey FOREIGN KEY (run_by) REFERENCES auth.users(id);

ALTER TABLE public.backtest_runs
  ADD CONSTRAINT backtest_runs_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

GRANT ALL ON public.backtest_runs TO anon;

GRANT ALL ON public.backtest_runs TO authenticated;

GRANT ALL ON public.backtest_runs TO service_role;

CREATE INDEX idx_backtest_runs_run_by ON public.backtest_runs (run_by);
...
ALTER TABLE public.bug_reports
  ADD CONSTRAINT bug_reports_status_check CHECK (status = ANY (ARRAY['OPEN'::text, 'IN_PROGRESS'::text, 'RESOLVED'::text, 'WONT_FIX'::text]));
...
CREATE POLICY admin_update_bug_reports ON public.bug_reports
  FOR UPDATE
  TO authenticated
  USING (public.is_current_user_admin())
...
ALTER TABLE public.candles
  ADD CONSTRAINT candles_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

GRANT ALL ON public.candles TO anon;

GRANT ALL ON public.candles TO authenticated;
...
CREATE TRIGGER trg_protect_idx_manual_candle
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.protect_idx_manual_candle();
...
GRANT ALL ON public.chart_analyses TO anon;

GRANT ALL ON public.chart_analyses TO authenticated;

GRANT ALL ON public.chart_analyses TO service_role;

CREATE INDEX idx_chart_analyses_stock_id ON public.chart_analyses (stock_id);

CREATE INDEX idx_chart_analyses_user_id ON public.chart_analyses (user_id);

CREATE POLICY chart_analyses_select_own ON public.chart_analyses
  FOR SELECT
  TO authenticated
  USING ((( SELECT auth.uid() AS uid) = user_id));

CREATE TABLE public.corporate_action_detections (
  id                           uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id                     uuid                     NOT NULL,
  news_event_id                uuid                     NOT NULL,
  detected_action_type         text,
  detection_method             text                     DEFAULT 'REGEX'::text NOT NULL,
  evidence_summary             text,
  status                       text                     DEFAULT 'NEW'::text NOT NULL,
  promoted_corporate_action_id uuid,
  reviewed_by                  uuid,
...
COMMENT ON TABLE public.corporate_action_detections IS 'Spec v5.0 section 18.1/18.3: staging area buat hasil deteksi AI+Regex dari news_events (event_type=AKSI_KORPORASI) sebelum admin promote ke corporate_actions. Berita untrusted, jadi tidak boleh langsung auto-invalidate signal.';

ALTER TABLE public.corporate_action_detections
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_news_event_id_key UNIQUE (news_event_id);

ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_pkey PRIMARY KEY (id);

ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_status_check CHECK (status = ANY (ARRAY['NEW'::text, 'PROMOTED'::text, 'DISMISSED'::text]));

GRANT ALL ON public.corporate_action_detections TO anon;

GRANT ALL ON public.corporate_action_detections TO authenticated;

GRANT ALL ON public.corporate_action_detections TO service_role;

CREATE INDEX idx_corporate_action_detections_stock_id ON public.corporate_action_detections (stock_id);

CREATE INDEX idx_corporate_action_detections_reviewed_by ON public.corporate_action_detections (reviewed_by);

CREATE INDEX idx_corporate_action_detections_promoted_action_id ON public.corporate_action_detections (promoted_corporate_action_id);

CREATE POLICY "admin can view detections" ON public.corporate_action_detections
  FOR SELECT
  TO authenticated
  USING (public.is_current_user_admin());
...
ALTER TABLE public.corporate_actions
  ADD CONSTRAINT corporate_actions_action_type_check
    CHECK (action_type = ANY (ARRAY['SPLIT'::text, 'REVERSE_SPLIT'::text, 'RIGHT_ISSUE'::text, 'DELISTING'::text, 'SUSPENSION'::text]));

ALTER TABLE public.corporate_actions
  ADD CONSTRAINT corporate_actions_pkey PRIMARY KEY (id);

ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_promoted_corporate_action_id_fkey FOREIGN KEY (promoted_corporate_action_id) REFERENCES public.corporate_actions(id);

ALTER TABLE public.corporate_actions
  ADD CONSTRAINT corporate_actions_status_check CHECK (status = ANY (ARRAY['PENDING'::text, 'PROCESSED'::text]));

GRANT ALL ON public.corporate_actions TO anon;

GRANT ALL ON public.corporate_actions TO authenticated;

GRANT ALL ON public.corporate_actions TO service_role;

CREATE INDEX corporate_actions_ex_date_status_idx ON public.corporate_actions (ex_date, status);

CREATE INDEX corporate_actions_stock_id_idx ON public.corporate_actions (stock_id);
...
ALTER TABLE public.earnings_calendar
  ADD CONSTRAINT earnings_calendar_status_check CHECK (status = ANY (ARRAY['SCHEDULED'::text, 'RELEASED'::text, 'DELAYED'::text]));

ALTER TABLE public.earnings_calendar
  ADD CONSTRAINT earnings_calendar_stock_quarter_year_key UNIQUE (stock_id, quarter, year);

GRANT ALL ON public.earnings_calendar TO anon;

GRANT ALL ON public.earnings_calendar TO authenticated;

GRANT ALL ON public.earnings_calendar TO service_role;

CREATE INDEX idx_earnings_calendar_stock_id ON public.earnings_calendar (stock_id);

CREATE POLICY "earnings_calendar readable by all" ON public.earnings_calendar
  FOR SELECT
  USING (true);
...
ALTER TABLE public.economic_events
  ADD CONSTRAINT economic_events_impact_check CHECK (impact = ANY (ARRAY['low'::text, 'medium'::text, 'high'::text]));
...
GRANT ALL ON public.economic_events TO anon;

GRANT ALL ON public.economic_events TO authenticated;
...
CREATE POLICY "economic_events readable by all" ON public.economic_events
  FOR SELECT
  USING (true);
...
ALTER TABLE public.feature_requests
  ADD CONSTRAINT feature_requests_status_check CHECK (status = ANY (ARRAY['OPEN'::text, 'PLANNED'::text, 'SHIPPED'::text, 'DECLINED'::text]));
...
CREATE POLICY "own feature requests insert" ON public.feature_requests
  FOR INSERT
  WITH CHECK ((( SELECT auth.uid() AS uid) = user_id));
...
CREATE TABLE public.foreign_flow (
  date       date                     DEFAULT ((now() AT TIME ZONE 'Asia/Jakarta'::text))::date NOT NULL,
  net_value  numeric                  DEFAULT 0 NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
);

ALTER TABLE public.foreign_flow
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.foreign_flow
  ADD CONSTRAINT foreign_flow_pkey PRIMARY KEY (date);

GRANT ALL ON public.foreign_flow TO anon;

GRANT ALL ON public.foreign_flow TO authenticated;

GRANT ALL ON public.foreign_flow TO service_role;

CREATE POLICY foreign_flow_select_all ON public.foreign_flow
  FOR SELECT
  USING (true);
...
GRANT ALL ON public.fundamentals TO anon;

GRANT ALL ON public.fundamentals TO authenticated;
...
CREATE TRIGGER trg_push_market_cap_to_quote
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.push_market_cap_to_quote();

CREATE POLICY "fundamentals readable by all" ON public.fundamentals
  FOR SELECT
  USING (true);

CREATE TABLE public.golden_test_cases (
  id                  uuid                     DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.golden_test_cases
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.golden_test_cases
  ADD CONSTRAINT golden_test_cases_case_code_check CHECK (case_code ~ '^GOLDEN-[0-9]{3}$'::text);
...
ALTER TABLE public.golden_test_cases
  ADD CONSTRAINT golden_test_cases_status_check CHECK (status = ANY (ARRAY['NOT_POPULATED'::text, 'READY'::text, 'PASSING'::text, 'FAILING'::text]));

GRANT ALL ON public.golden_test_cases TO anon;

GRANT ALL ON public.golden_test_cases TO authenticated;

GRANT ALL ON public.golden_test_cases TO service_role;
...
CREATE TABLE public.idx_eod_uploads (
  id                uuid                     DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.idx_eod_uploads
  ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON public.idx_eod_uploads TO anon;

GRANT ALL ON public.idx_eod_uploads TO authenticated;

GRANT ALL ON public.idx_eod_uploads TO service_role;
...
ALTER TABLE public.indicators
  ADD CONSTRAINT indicators_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

GRANT ALL ON public.indicators TO anon;

GRANT ALL ON public.indicators TO authenticated;
...
ALTER TABLE public.internal_secrets
  ENABLE ROW LEVEL SECURITY;
...
CREATE TABLE public.intraday_evaluations (
  id                    uuid                     DEFAULT gen_random_uuid() NOT NULL,
...
GRANT ALL ON public.intraday_evaluations TO anon;

GRANT ALL ON public.intraday_evaluations TO authenticated;
...
GRANT ALL ON public.ipo_calendar TO authenticated;

GRANT ALL ON public.ipo_calendar TO service_role;

CREATE UNIQUE INDEX ipo_calendar_ticker_unique ON public.ipo_calendar (ticker)
  WHERE ticker IS NOT NULL;

CREATE POLICY "ipo_calendar readable by all" ON public.ipo_calendar
  FOR SELECT
  USING (true);
...
ALTER TABLE public.job_runs
  ADD CONSTRAINT job_runs_status_check CHECK (status = ANY (ARRAY['RUNNING'::text, 'SUCCESS'::text, 'ERROR'::text, 'FAILED'::text]));

GRANT ALL ON public.job_runs TO anon;

GRANT ALL ON public.job_runs TO authenticated;

GRANT ALL ON public.job_runs TO service_role;

CREATE POLICY admin_select_job_runs ON public.job_runs
  FOR SELECT
  TO authenticated
  USING (public.is_current_user_admin());
...
GRANT ALL ON public.market_calendar TO anon;

GRANT ALL ON public.market_calendar TO authenticated;

GRANT ALL ON public.market_calendar TO service_role;

CREATE POLICY "market_calendar readable by all" ON public.market_calendar
  FOR SELECT
  USING (true);
...
ALTER TABLE public.market_index
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.market_index
  ADD CONSTRAINT market_index_quality_check CHECK (quality = ANY (ARRAY['FRESH'::text, 'STALE'::text, 'MISSING'::text, 'INVALID'::text]));

GRANT ALL ON public.market_index TO anon;

GRANT ALL ON public.market_index TO authenticated;

GRANT ALL ON public.market_index TO service_role;

CREATE POLICY market_index_select_all ON public.market_index
  FOR SELECT
  USING (true);

CREATE TABLE public.mtf_pipeline_runs (
  id                 uuid                     DEFAULT gen_random_uuid() NOT NULL,
  tier               text                     NOT NULL,
...
ALTER TABLE public.mtf_pipeline_runs
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.mtf_pipeline_runs
  ADD CONSTRAINT mtf_pipeline_runs_pkey PRIMARY KEY (id);

GRANT ALL ON public.mtf_pipeline_runs TO anon;

GRANT ALL ON public.mtf_pipeline_runs TO authenticated;

GRANT ALL ON public.mtf_pipeline_runs TO service_role;

CREATE INDEX idx_mtf_pipeline_runs_status ON public.mtf_pipeline_runs (status);
...
ALTER TABLE public.news
  ADD CONSTRAINT news_sentiment_check CHECK (sentiment = ANY (ARRAY['positive'::text, 'neutral'::text, 'negative'::text]));
...
ALTER TABLE public.news_events
  ADD CONSTRAINT news_events_impact_classification_check
    CHECK (impact_classification = ANY (ARRAY['POTENTIALLY_POSITIVE'::text, 'POTENTIALLY_NEGATIVE'::text, 'MIXED_UNCLEAR'::text, 'INFORMATIONAL'::text]));

ALTER TABLE public.news_events
  ADD CONSTRAINT news_events_pkey PRIMARY KEY (id);

ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_news_event_id_fkey FOREIGN KEY (news_event_id) REFERENCES public.news_events(id);

ALTER TABLE public.news_events
  ADD CONSTRAINT news_events_primary_article_id_fkey FOREIGN KEY (primary_article_id) REFERENCES public.news(id);

GRANT ALL ON public.news_events TO anon;
...
CREATE TABLE public.news_issuer_mapping (
  id                 uuid                     DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.news_issuer_mapping
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.news_issuer_mapping
  ADD CONSTRAINT news_issuer_mapping_impact_check
    CHECK (impact = ANY (ARRAY['POTENTIALLY_POSITIVE'::text, 'POTENTIALLY_NEGATIVE'::text, 'MIXED_UNCLEAR'::text, 'INFORMATIONAL'::text]));

ALTER TABLE public.news_issuer_mapping
  ADD CONSTRAINT news_issuer_mapping_mapping_confidence_check CHECK (mapping_confidence = ANY (ARRAY['HIGH'::text, 'MEDIUM'::text, 'LOW'::text]));

ALTER TABLE public.news_issuer_mapping
  ADD CONSTRAINT news_issuer_mapping_mapping_method_check
    CHECK
    (mapping_method = ANY (ARRAY['EXPLICIT_TICKER'::text, 'EXPLICIT_ISSUER_NAME'::text, 'CORPORATE_ACTION'::text, 'ENTITY_MATCH'::text, 'INDUSTRY_CONTEXT'::text,
    'LOCATION_CONTEXT'::text]));

ALTER TABLE public.news_issuer_mapping
  ADD CONSTRAINT news_issuer_mapping_pkey PRIMARY KEY (id);

GRANT ALL ON public.news_issuer_mapping TO anon;

GRANT ALL ON public.news_issuer_mapping TO authenticated;

GRANT ALL ON public.news_issuer_mapping TO service_role;

CREATE INDEX idx_news_issuer_mapping_article ON public.news_issuer_mapping (article_id);

CREATE INDEX idx_news_issuer_mapping_stock ON public.news_issuer_mapping (stock_id);
...
ALTER TABLE public.notifications
  ADD CONSTRAINT notifications_category_check
    CHECK (category = ANY (ARRAY['MARKET'::text, 'SIGNAL'::text, 'NEWS'::text, 'ECONOMIC_EVENT'::text, 'MORNING_BRIEFING'::text, 'UNUSUAL_ACTIVITY'::text, 'SUBSCRIPTION'::text]));
...
GRANT ALL ON public.payment_events TO anon;
...
ALTER TABLE public.payments
  ADD CONSTRAINT payments_method_check CHECK (method = ANY (ARRAY['QRIS'::text, 'VA_BANK'::text, 'DANA'::text, 'OVO'::text, 'GOPAY'::text, 'SHOPEEPAY'::text]));
...
ALTER TABLE public.payments
  ADD CONSTRAINT payments_status_check CHECK (status = ANY (ARRAY['pending'::text, 'success'::text, 'expired'::text, 'failed'::text]));
...
CREATE POLICY "admin read ai_usage" ON public.ai_usage
  FOR SELECT
  USING ((EXISTS ( SELECT 1
   FROM public.profiles p
  WHERE ((p.id = ( SELECT auth.uid() AS uid)) AND (p.is_admin = true)))));

CREATE POLICY "backtest_gate_config readable by admin" ON public.backtest_gate_config
  FOR SELECT
  TO authenticated
  USING ((EXISTS ( SELECT 1
   FROM public.profiles p
...
CREATE POLICY "backtest_runs readable by admin" ON public.backtest_runs
  FOR SELECT
  TO authenticated
  USING ((EXISTS ( SELECT 1
   FROM public.profiles p
...
ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES public.profiles(id);
...
GRANT UPDATE (full_name, risk_profile) ON public.profiles TO authenticated;
...
CREATE POLICY profiles_select ON public.profiles
  FOR SELECT
  USING (((( SELECT auth.uid() AS uid) = id) OR public.is_current_user_admin()));
...
ALTER TABLE public.provider_health
  ADD CONSTRAINT provider_health_status_check CHECK (status = ANY (ARRAY['UP'::text, 'DEGRADED'::text, 'DOWN'::text]));

GRANT ALL ON public.provider_health TO anon;

GRANT ALL ON public.provider_health TO authenticated;

GRANT ALL ON public.provider_health TO service_role;

CREATE INDEX idx_provider_health_provider_checked ON public.provider_health (PROVIDER, checked_at DESC);

CREATE POLICY admin_select_provider_health ON public.provider_health
  FOR SELECT
  TO authenticated
  USING (public.is_current_user_admin());
...
ALTER TABLE public.quotes
  ADD CONSTRAINT quotes_quality_check CHECK (quality = ANY (ARRAY['FRESH'::text, 'STALE'::text, 'MISSING'::text, 'INVALID'::text]));
...
CREATE TRIGGER trg_protect_idx_manual_quote
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.protect_idx_manual_quote();

CREATE TRIGGER trg_sync_quote_market_cap
  BEFORE INSERT OR UPDATE ON public.quotes
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_quote_market_cap();
...
CREATE INDEX idx_saved_signals_user_id ON public.saved_signals (user_id);
...
ALTER TABLE public.sector_rotation_scores
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.sector_rotation_scores
  ADD CONSTRAINT sector_rotation_scores_label_check CHECK (label = ANY (ARRAY['LEADING'::text, 'IMPROVING'::text, 'WEAKENING'::text, 'LAGGING'::text]));
...
GRANT ALL ON public.sector_rotation_scores TO anon;

GRANT ALL ON public.sector_rotation_scores TO authenticated;

GRANT ALL ON public.sector_rotation_scores TO service_role;

CREATE INDEX sector_rotation_scores_date_idx ON public.sector_rotation_scores (as_of_date);

CREATE POLICY "public read of sector rotation scores" ON public.sector_rotation_scores
  FOR SELECT
  TO anon, authenticated
  USING (true);
...
CREATE TABLE public.security_acceptance_checks (
  id                uuid                     DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.security_acceptance_checks
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.security_acceptance_checks
  ADD CONSTRAINT security_acceptance_checks_test_status_check CHECK (test_status = ANY (ARRAY['NOT_TESTED'::text, 'PASS'::text, 'FAIL'::text]));

ALTER TABLE public.security_acceptance_checks
  ADD CONSTRAINT security_acceptance_checks_threat_key UNIQUE (threat);

GRANT ALL ON public.security_acceptance_checks TO anon;

GRANT ALL ON public.security_acceptance_checks TO authenticated;

GRANT ALL ON public.security_acceptance_checks TO service_role;

CREATE INDEX idx_security_acceptance_checks_last_tested_by ON public.security_acceptance_checks (last_tested_by);
...
CREATE TABLE public.session2_setup_previews (
  id                   uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id             uuid                     NOT NULL,
  trade_date           date                     NOT NULL,
...
ALTER TABLE public.session2_setup_previews
  ADD CONSTRAINT session2_setup_previews_direction_bias_check CHECK (direction_bias = ANY (ARRAY['POTENTIAL_BUY'::text, 'POTENTIAL_SELL'::text, 'NONE'::text]));

ALTER TABLE public.session2_setup_previews
  ADD CONSTRAINT session2_setup_previews_pkey PRIMARY KEY (id);

ALTER TABLE public.session2_setup_previews
  ADD CONSTRAINT session2_setup_previews_stock_id_trade_date_key UNIQUE (stock_id, trade_date);

GRANT ALL ON public.session2_setup_previews TO anon;

GRANT ALL ON public.session2_setup_previews TO authenticated;

GRANT ALL ON public.session2_setup_previews TO service_role;

CREATE INDEX idx_session2_setup_previews_nearby_zone_id ON public.session2_setup_previews (nearby_zone_id);

CREATE POLICY "session2_setup_previews readable by all" ON public.session2_setup_previews
  FOR SELECT
  USING (true);

CREATE TABLE public.signal_engine_versions (
  id              uuid                     DEFAULT gen_random_uuid() NOT NULL,
  formula_version text                     NOT NULL,
  timeframe       text                     NOT NULL,
...
ALTER TABLE public.signal_engine_versions
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.signal_engine_versions
  ADD CONSTRAINT signal_engine_versions_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

GRANT ALL ON public.signal_engine_versions TO anon;

GRANT ALL ON public.signal_engine_versions TO authenticated;

GRANT ALL ON public.signal_engine_versions TO service_role;

CREATE INDEX idx_signal_engine_versions_backtest_run_id ON public.signal_engine_versions (backtest_run_id);

CREATE POLICY "signal_engine_versions readable by all" ON public.signal_engine_versions
  FOR SELECT
  USING (true);
...
GRANT ALL ON public.signal_evidence TO anon;

GRANT ALL ON public.signal_evidence TO authenticated;

GRANT ALL ON public.signal_evidence TO service_role;

CREATE INDEX idx_signal_evidence_signal_id ON public.signal_evidence (signal_id);

CREATE POLICY "admin read full signal_evidence" ON public.signal_evidence
  FOR SELECT
  TO authenticated
  USING (public.is_current_user_admin());

CREATE TABLE public.signal_pipeline_events (
  id         uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id   uuid,
  tier       text,
  stage      text                     NOT NULL,
...
ALTER TABLE public.signal_pipeline_events
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.signal_pipeline_events
  ADD CONSTRAINT signal_pipeline_events_stage_check
    CHECK
    (stage = ANY (ARRAY['ingestion'::text, 'quality'::text, 'indicator'::text, 'structure'::text, 'confluence'::text, 'signal'::text, 'snapshot'::text, 'notification'::text]));

ALTER TABLE public.signal_pipeline_events
  ADD CONSTRAINT signal_pipeline_events_status_check CHECK (status = ANY (ARRAY['OK'::text, 'FAILED'::text, 'SKIPPED'::text]));

ALTER TABLE public.signal_pipeline_events
  ADD CONSTRAINT signal_pipeline_events_tier_check CHECK (tier = ANY (ARRAY['daily'::text, 'swing'::text]));

GRANT ALL ON public.signal_pipeline_events TO anon;

GRANT ALL ON public.signal_pipeline_events TO authenticated;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON public.signal_results TO anon;

GRANT DELETE, INSERT, MAINTAIN, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON public.signal_results TO authenticated;

GRANT ALL ON public.signal_results TO service_role;

CREATE POLICY "signal_results readable by all" ON public.signal_results
  FOR SELECT
  USING (true);
...
ALTER TABLE public.signal_revisions
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.signal_revisions
  ADD CONSTRAINT signal_revisions_approval_status_check CHECK (approval_status = ANY (ARRAY['PENDING'::text, 'APPROVED'::text, 'REJECTED'::text]));
...
GRANT ALL ON public.signal_revisions TO anon;

GRANT ALL ON public.signal_revisions TO authenticated;
...
ALTER TABLE public.signal_unlocks
  ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.signal_unlocks
  ADD CONSTRAINT signal_unlocks_source_check CHECK (source = ANY (ARRAY['TOKEN'::text, 'AD'::text, 'PREMIUM'::text, 'ADMIN'::text]));
...
GRANT ALL ON public.signal_unlocks TO anon;

GRANT ALL ON public.signal_unlocks TO authenticated;
...
CREATE TABLE public.signal_watch_states (
  id               uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id         uuid                     NOT NULL,
...
ALTER TABLE public.signal_watch_states
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_bias_check CHECK (bias = ANY (ARRAY['bullish'::text, 'bearish'::text]));

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_pkey PRIMARY KEY (id);

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_signal_tier_check CHECK (signal_tier = ANY (ARRAY['daily'::text, 'swing'::text]));

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_state_check CHECK (state = ANY (ARRAY['WATCHLIST'::text, 'NETRAL'::text]));

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_stock_id_signal_tier_key UNIQUE (stock_id, signal_tier);

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_watch_direction_check CHECK (watch_direction = ANY (ARRAY['BUY'::text, 'SELL'::text]));

ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_watch_zone_type_check CHECK (watch_zone_type = ANY (ARRAY['SUPPORT'::text, 'RESISTANCE'::text]));

GRANT ALL ON public.signal_watch_states TO anon;

GRANT ALL ON public.signal_watch_states TO authenticated;

GRANT ALL ON public.signal_watch_states TO service_role;

CREATE INDEX idx_signal_watch_states_tier_state ON public.signal_watch_states (signal_tier, state);

CREATE POLICY "watch states readable by all authenticated" ON public.signal_watch_states
  FOR SELECT
  TO authenticated
  USING (true);
...
COMMENT ON COLUMN public.signals.level_mode IS 'Spec v5.0 section 7.4: SCALP (D1 terdekat, RR kecil) / STANDARD (D1 berikutnya, min 2 touch) / SWING (W1, RR besar). NULL untuk signal lama sebelum kolom ini ada.';

ALTER TABLE public.signals
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.signals
  ADD CONSTRAINT signals_bearish_type_check
    CHECK
    (bearish_type IS NULL OR (bearish_type = ANY (ARRAY['BEARISH_REJECTION'::text, 'BEARISH_BREAKDOWN'::text, 'BEARISH_DISTRIBUTION_INDICATION'::text,
    'BEARISH_CONTINUATION'::text])));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_data_source_check CHECK (data_source IS NULL OR (data_source = ANY (ARRAY['IDX'::text, 'YAHOO_FALLBACK'::text])));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_direction_check CHECK (direction = ANY (ARRAY['BUY'::text, 'SELL'::text]));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_entry_type_check CHECK (entry_type IS NULL OR (entry_type = ANY (ARRAY['AREA'::text, 'TRIGGER'::text, 'RETEST'::text, 'SUPPORT'::text])));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_level_mode_check CHECK (level_mode IS NULL OR (level_mode = ANY (ARRAY['SCALP'::text, 'STANDARD'::text, 'SWING'::text])));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_overextension_status_check
    CHECK (overextension_status IS NULL OR (overextension_status = ANY (ARRAY['NORMAL'::text, 'OVEREXTENDED'::text, 'UNKNOWN'::text])));
...
ALTER TABLE public.signals
  ADD CONSTRAINT signals_signal_tier_check CHECK (signal_tier = ANY (ARRAY['daily'::text, 'swing'::text]));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_status_check
    CHECK (status = ANY (ARRAY['ACTIVE'::text, 'HIT_TP1'::text, 'HIT_TP2'::text, 'HIT_SL'::text, 'HIT_SL_LOCKED'::text, 'EXPIRED'::text, 'INVALIDATED'::text]));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_superseded_by_fkey FOREIGN KEY (superseded_by) REFERENCES public.signals(id);

ALTER TABLE public.signals
  ADD CONSTRAINT signals_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

ALTER TABLE public.signals
  ADD CONSTRAINT signals_trigger_state_check
    CHECK
    (trigger_state IS NULL OR (trigger_state = ANY (ARRAY['INTRABAR_TOUCH'::text, 'BREAKOUT_CANDIDATE'::text, 'BREAKOUT_CONFIRMED'::text, 'FAILED_BREAKOUT'::text,
    'BREAKOUT_RETEST'::text, 'BREAKDOWN_CANDIDATE'::text, 'BREAKDOWN_CONFIRMED'::text, 'FAILED_BREAKDOWN'::text, 'BREAKDOWN_RETEST'::text])));

GRANT SELECT (created_at, direction, id, resolved_at, status, stock_id, superseded_by, timeframe) ON public.signals TO anon;

GRANT SELECT (created_at, direction, id, resolved_at, status, stock_id, superseded_by, timeframe) ON public.signals TO authenticated;
...
CREATE TRIGGER trg_signal_candle_freshness
  BEFORE INSERT ON public.signals
  FOR EACH ROW
  EXECUTE FUNCTION public.assert_signal_candles_fresh();
...
CREATE POLICY "read full signals (admin or unlocked)" ON public.signals
  FOR SELECT
  TO authenticated
  USING ((public.is_current_user_admin() OR (stock_id IN ( SELECT signal_unlocks.stock_id
   FROM public.signal_unlocks
  WHERE (signal_unlocks.user_id = ( SELECT auth.uid() AS uid))))));
...
CREATE TABLE public.stocks (
  ...
  trending_reason_attempts   integer                  DEFAULT 0 NOT NULL,
  trending_reason_last_error text
);
...
ALTER TABLE public.corporate_action_detections
  ADD CONSTRAINT corporate_action_detections_stock_id_fkey FOREIGN KEY (stock_id) REFERENCES public.stocks(id);
...
ALTER TABLE public.signal_watch_states
  ADD CONSTRAINT signal_watch_states_stock_id_fkey FOREIGN KEY (stock_id) REFERENCES public.stocks(id) ON DELETE CASCADE;
...
CREATE TABLE public.structure_labels (
  id               uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id         uuid                     NOT NULL,
  timeframe        text                     NOT NULL,
  label            text                     NOT NULL,
...
ALTER TABLE public.structure_labels
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.structure_labels
  ADD CONSTRAINT structure_labels_label_check
    CHECK (label = ANY (ARRAY['HH'::text, 'HL'::text, 'LH'::text, 'LL'::text, 'SWING_HIGH'::text, 'SWING_LOW'::text, 'SIDEWAYS'::text, 'UNCLEAR'::text]));
...
ALTER TABLE public.structure_labels
  ADD CONSTRAINT structure_labels_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

GRANT ALL ON public.structure_labels TO anon;

GRANT ALL ON public.structure_labels TO authenticated;

GRANT ALL ON public.structure_labels TO service_role;

CREATE INDEX idx_structure_labels_source_candle_id ON public.structure_labels (source_candle_id);

CREATE INDEX idx_structure_labels_stock_tf ON public.structure_labels (stock_id, timeframe, detected_at DESC);

CREATE POLICY "structure_labels readable by all" ON public.structure_labels
  FOR SELECT
  USING (true);

CREATE TABLE public.structure_zones (
  id                      uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id                uuid                     NOT NULL,
  timeframe               text                     NOT NULL,
...
ALTER TABLE public.structure_zones
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_age_in_bars_check CHECK (age_in_bars >= 0);

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_check CHECK (price_high >= price_low);
...
ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_retest_count_check CHECK (retest_count >= 0);

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_stock_id_fkey FOREIGN KEY (stock_id) REFERENCES public.stocks(id);

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_strength_check CHECK (strength = ANY (ARRAY['WEAK'::text, 'MODERATE'::text, 'STRONG'::text]));

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_timeframe_check CHECK (timeframe = ANY (ARRAY['D1'::text, 'W1'::text]));

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_touch_count_check CHECK (touch_count >= 1);

ALTER TABLE public.structure_zones
  ADD CONSTRAINT structure_zones_zone_type_check CHECK (zone_type = ANY (ARRAY['SUPPORT'::text, 'RESISTANCE'::text]));

GRANT ALL ON public.structure_zones TO anon;

GRANT ALL ON public.structure_zones TO authenticated;
...
ALTER TABLE public.subscriptions
  ADD CONSTRAINT subscriptions_status_check CHECK (status = ANY (ARRAY['active'::text, 'pending_cancel'::text, 'grace'::text, 'expired'::text]));
...
CREATE UNIQUE INDEX token_transactions_debit_type_reference_id_uniq ON public.token_transactions (TYPE, reference_id)
...
CREATE UNIQUE INDEX token_transactions_credit_type_reference_id_uniq ON public.token_transactions (TYPE, reference_id)
  WHERE reference_id IS NOT NULL AND amount > 0;
...
CREATE FUNCTION public.ensure_wallet_current (
  p_user_id uuid
)
...
begin
  ...
  if not found then
    ...
    insert into public.token_transactions (wallet_id, amount, type, balance_before, balance_after)
      values (v_wallet.id, v_wallet.balance, 'DAILY_GRANT', 0, v_wallet.balance);
  end if;

  select coalesce(is_admin, false) into v_is_admin from public.profiles where id = p_user_id;

  -- Admin/founder: keep the underlying row permanently topped up, skip normal
  -- daily-grant reset logic entirely. Ledger stays auditable via ADMIN_ADJUSTMENT.
  if v_is_admin then
    if v_wallet.balance <> v_admin_target then
      v_balance_before := v_wallet.balance;

      update public.token_wallets
        set balance = v_admin_target
        where id = v_wallet.id
        returning * into v_wallet;

      insert into public.token_transactions (wallet_id, amount, type, balance_before, balance_after)
        values (v_wallet.id, v_admin_target - v_balance_before, 'ADMIN_ADJUSTMENT', v_balance_before, v_admin_target);
    end if;
  ...
  v_period := case when v_is_premium then 3 else 1 end;

  if v_today - v_wallet.last_reset_date >= v_period then
    v_balance_before := v_wallet.balance;
    ...
    insert into public.token_transactions (wallet_id, amount, type, balance_before, balance_after)
      values (v_wallet.id, v_wallet.balance - v_balance_before, 'DAILY_GRANT', v_balance_before, v_wallet.balance);
  end if;
...
REVOKE ALL ON FUNCTION public.ensure_wallet_current(uuid) FROM PUBLIC;

GRANT ALL ON FUNCTION public.ensure_wallet_current(uuid) TO service_role;
...
CREATE POLICY "own token transactions select" ON public.token_transactions
  FOR SELECT
  USING ((EXISTS ( SELECT 1
   FROM public.token_wallets tw
...
ALTER TABLE public.token_wallets
  ADD CONSTRAINT token_wallets_balance_nonneg CHECK (balance >= 0);
...
CREATE POLICY "own token wallet select" ON public.token_wallets
  FOR SELECT
  USING ((( SELECT auth.uid() AS uid) = user_id));
...
GRANT ALL ON public.trading_plans TO anon;

GRANT ALL ON public.trading_plans TO authenticated;

GRANT ALL ON public.trading_plans TO service_role;

CREATE TRIGGER trading_plan_tier_check
  BEFORE INSERT OR UPDATE ON public.trading_plans
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_trading_plan_tier();
...
CREATE TABLE public.unusual_activities (
  id                   uuid                     DEFAULT gen_random_uuid() NOT NULL,
  stock_id             uuid                     NOT NULL,
...
ALTER TABLE public.unusual_activities
  ADD CONSTRAINT unusual_activities_severity_check CHECK (severity = ANY (ARRAY['LOW'::text, 'MEDIUM'::text, 'HIGH'::text]));

ALTER TABLE public.unusual_activities
  ADD CONSTRAINT unusual_activities_stock_id_fkey FOREIGN KEY (stock_id) REFERENCES public.stocks(id) ON DELETE CASCADE;

GRANT ALL ON public.unusual_activities TO anon;

GRANT ALL ON public.unusual_activities TO authenticated;

GRANT ALL ON public.unusual_activities TO service_role;

CREATE INDEX idx_unusual_activities_stock_id ON public.unusual_activities (stock_id);
...
CREATE POLICY "unusual_activities readable by all" ON public.unusual_activities
  FOR SELECT
  USING (true);
...
CREATE INDEX idx_watchlist_items_stock_id ON public.watchlist_items (stock_id);

CREATE TRIGGER trg_enforce_watchlist_item_limit
  BEFORE INSERT ON public.watchlist_items
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_watchlist_item_limit();
...
CREATE TRIGGER trg_enforce_watchlist_folder_limit
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_watchlist_folder_limit();

CREATE POLICY "own watchlists" ON public.watchlists
  USING ((( SELECT auth.uid() AS uid) = user_id));

CREATE VIEW public.signals_public WITH (security_invoker=true) AS SELECT id,
    stock_id,
    direction,
    timeframe,
    status,
    ...
    bearish_type,
        CASE
            WHEN (direction = 'BUY'::text) THEN buy_area_low
            ELSE NULL::numeric
        END AS buy_area_low,
        CASE
            WHEN (direction = 'BUY'::text) THEN buy_area_high
            ELSE NULL::numeric
        END AS buy_area_high,
        CASE
            WHEN (direction = 'SELL'::text) THEN downside_support_1
            ELSE NULL::numeric
        END AS downside_support_1
   FROM public.signals s;

GRANT ALL ON public.signals_public TO anon;

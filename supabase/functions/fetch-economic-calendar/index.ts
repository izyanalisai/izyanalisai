...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    return new Response(JSON.stringify({
      error: `gagal ambil feed: ${String(e)}`
    }), {
      status: 502
    });
  }
  ...
  const rows = events.filter((e)=>e.title && e.date && e.country && e.impact !== 'Holiday').map((e)=>{
  ...
  for (const row of rows){
    const { error } = await supabase.from('economic_events').upsert(row, {
      onConflict: 'event_name,country,event_date'
    });
    if (error) {
      failed++;

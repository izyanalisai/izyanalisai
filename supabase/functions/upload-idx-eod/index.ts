...
const MY_TEST_TOKEN = 'izy-local-test-8f3k2q';
...
async function processRows(rows, tradeDateInput, admin, userId, fileName) {
  ...
  const unmatched = [];
  let matched = 0;
  // Foreign flow (Section: aggregate market-wide net foreign value from
  // the 'Foreign Buy' / 'Foreign Sell' columns already present in IDX's
  // official daily 'Ringkasan Saham' export - no new data source needed).
...
Deno.serve(async (req)=>{
  ...
  const testBypass = req.headers.get('x-internal-test');
  let userId = null;
  if (testBypass === MY_TEST_TOKEN) {
    const body = await req.json();
    const bin = Uint8Array.from(atob(body.file_base64), (c)=>c.charCodeAt(0));
    let wb;
    try {
      wb = XLSX.read(bin, {
        type: 'array'
    ...
    try {
      const result = await processRows(rows, body.trade_date ?? null, admin, null, body.file_name ?? 'test.xlsx');
  ...
  let wb2;
  try {
    wb2 = XLSX.read(new Uint8Array(buf), {
      type: 'array'
    });
  } catch (e) {
    return new Response(JSON.stringify({
  ...
  if (rows2.length === 0) return new Response(JSON.stringify({
    error: 'no rows found in file'
  }), {
    status: 400
  });
  try {
    const result = await processRows(rows2, tradeDateInput, admin, userId, file.name);
    return new Response(JSON.stringify(result, null, 2), {
      headers: {
        'Content-Type': 'application/json'
      }

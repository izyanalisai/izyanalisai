...
Deno.serve(async (_req)=>{
  ...
  const { data: quotes, error: qErr } = await supabase.from('quotes').select('stock_id, price, previous_close, volume').not('volume', 'is', null).limit(2000);
  if (qErr) return new Response(JSON.stringify({
    error: qErr.message
  }), {
    status: 500
  });
  const { data: indicators, error: iErr } = await supabase.from('indicators').select('stock_id, volume_avg20, ts').eq('timeframe', 'D1').not('volume_avg20', 'is', null).order('ts', {
    ascending: false
  }).limit(4000);
  if (iErr) return new Response(JSON.stringify({
    error: iErr.message
  }), {
    status: 500
  });
  ...
  const { data: recent } = await supabase.from('unusual_activities').select('stock_id').gt('timestamp', cutoff).in('stock_id', detections.map((d)=>d.stock_id));
  ...
  const rows = fresh.map((d)=>{
    ...
    return {
      ...
      price: d.price,
      volume: d.volume,
      avg_volume_20d: d.avg,
      price_change_percent: Number(d.changePct.toFixed(2)),
  ...
  if (insErr) return new Response(JSON.stringify({
    error: insErr.message
  }), {
    status: 500
  });
  // Notif real-time hanya untuk user Premium + toggle unusual_activity_alert aktif (poin 7.2)
  const { data: recipients } = await supabase.from('profiles').select('id, notification_preferences!inner(master_enabled, unusual_activity_alert)').eq('is_premium', true).eq('notification_preferences.master_enabled', true).eq('notification_preferences.unusual_activity_alert', true);
  let notified = 0;
  if (recipients && recipients.length > 0 && inserted) {
    for (const act of inserted){
      const notifRows = recipients.map((r)=>({
          ...
          reference_id: act.stock_id,
          event_id: `unusual:${act.id}:${r.id}`
        }));

...
function macd(closes) {
  ...
  signalOnValid.forEach((v, idx)=>{
    if (v != null) signalLine[validIdx[idx]] = v;
  });
...
function stochastic(highs, lows, closes, period = 14, smoothK = 3, smoothD = 3) {
  ...
  const kSmoothed = new Array(closes.length).fill(null);
  for(let i = 0; i < closes.length; i++){
    const window = rawK.slice(Math.max(0, i - smoothK + 1), i + 1).filter((v)=>v != null);
    if (window.length === smoothK) kSmoothed[i] = window.reduce((a, b)=>a + b, 0) / smoothK;
  }
  const dSmoothed = new Array(closes.length).fill(null);
  for(let i = 0; i < closes.length; i++){
    const window = kSmoothed.slice(Math.max(0, i - smoothD + 1), i + 1).filter((v)=>v != null);
...
Deno.serve(async (req)=>{
  ...
  for(let i = 0; i < stocks.length; i += CONCURRENCY){
    ...
    const results = await Promise.allSettled(batch.map(async (s)=>{
      const { data: candles, error: cErr } = await supabase.from('candles').select('ts, high, low, close, volume').eq('stock_id', s.id).eq('timeframe', timeframe).order('ts', {
        ascending: true
      });
      if (cErr) throw cErr;
      return {
    ...
    for (const r of results){
      ...
      const last20Vol = volumes.slice(-20).filter((v)=>v != null);
    ...
    if (rowsToUpsert.length > 0) {
      const { error: upsertErr } = await supabase.from('indicators').upsert(rowsToUpsert, {
        onConflict: 'stock_id,timeframe'
      });
      if (upsertErr) {

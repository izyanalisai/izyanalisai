...
async function fetchFundamentals(ticker, crumb, cookie) {
  const res = await fetch(`${YAHOO_QUOTESUMMARY_BASE}/${ticker}.JK?modules=defaultKeyStatistics,summaryDetail,price&crumb=${encodeURIComponent(crumb)}`, {
    headers: {
      'User-Agent': 'Mozilla/5.0',
      'Cookie': cookie
    }
...
Deno.serve(async (req)=>{
  ...
  for(let i = 0; i < stocks.length; i += CONCURRENCY){
    ...
    const results = await Promise.allSettled(batch.map(async (s)=>({
        stock: s,
        data: await fetchFundamentals(s.ticker, crumb, cookie)
      })));
    const rows = [];
    ...
    if (rows.length > 0) {
      const { error: upsertError } = await supabase.from('fundamentals').upsert(rows, {
        onConflict: 'stock_id'
      });
      if (upsertError) {

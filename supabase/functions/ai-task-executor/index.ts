...
Deno.serve(async (_req)=>{
  ...
  const { data: tasks, error } = await supabase.from('ai_tasks').select('id, user_id, task_type, parameters, last_run').eq('is_active', true).eq('status', 'ACTIVE').limit(500);
  ...
  const userIds = [
    ...new Set(tasks.map((t)=>t.user_id))
  ];
  const { data: prefs } = await supabase.from('notification_preferences').select('user_id, master_enabled, signal_alerts, morning_briefing, unusual_activity_alert').in('user_id', userIds);
  const prefMap = new Map((prefs ?? []).map((p)=>[
      p.user_id,
      p
    ]));
  function allowed(userId, key) {
      ...
      if (t.task_type === 'PRICE_ALERT') {
        ...
        if (hit && allowed(t.user_id, 'signal_alerts')) {
          await supabase.from('notifications').insert({
            ...
            event_id: `ai_task:${t.id}:price_alert`
          });
        ...
        if (diffPct <= tolerancePct && allowed(t.user_id, 'signal_alerts')) {
          await supabase.from('notifications').insert({
            ...
            reference_id: stock.id,
            event_id: `ai_task:${t.id}:level_retest`
          });
          await supabase.from('ai_tasks').update({
            status: 'DONE',
            is_active: false,
        ...
        const changeTxt = idx?.previous_close && idx?.value != null ? `${Number(idx.value) - Number(idx.previous_close) >= 0 ? '+' : ''}${(Number(idx.value) - Number(idx.previous_close)).toFixed(2)} poin` : 'data belum tersedia';
        ...
        let query = supabase.from('unusual_activities').select('id, stock_id, price, volume, price_change_percent, timestamp').gt('timestamp', since).order('timestamp', {
          ascending: false
        }).limit(5);
        if (ticker) {
          const { data: stock } = await supabase.from('stocks').select('id').eq('ticker', ticker).maybeSingle();
        ...
        if (activities && activities.length > 0 && allowed(t.user_id, 'unusual_activity_alert')) {
          for (const a of activities){
            await supabase.from('notifications').insert({
              ...
              reference_id: a.stock_id,
              event_id: `ai_task:${t.id}:unusual:${a.id}`
            });
          }
          fired++;
        }

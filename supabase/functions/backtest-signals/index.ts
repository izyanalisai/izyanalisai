...
const FORMULA_VERSION = 'breakeven_runner_v12';
const ATR_PERIOD = 14;
const ADX_PERIOD = 14;
...
function classifyTier(avgDailyValue) {
...
function adxSeries(candles, period = ADX_PERIOD) {
  ...
  const firstWindow = dxArr.slice(startIdx, firstWindowEnd).filter((v)=>v != null);
...
function candlestickScore(c0, c1, c2) {
  ...
  return {
    score: 0,
    label: 'tidak ada pola signifikan'
  };
}
function scoreSignal(ind, lastCandle, prevClose, ema21Now, ema21Before, candle, adxNow) {
...
// v12 (breakeven-stop + let-winner-run, 15 Agustus 2026):
// Root cause WR/PF jelek di v11 BUKAN di indikator/scoring, tapi di exit
// management: dulu cuma ada 2 hasil (kena TP1=1.5R full, atau kena SL full
// loss). Banyak trade yang sempat profit lumayan lalu balik arah dan kena SL
// penuh -- itu yang menggerus win rate & profit factor di semua tier.
// Fix: begitu harga sudah bergerak +1R dari entry, SL dipindah ke breakeven
// (entry + fee buffer) dan target dinaikkan ke TP2=3R (biarkan profit
// berjalan). Trade yang tadinya "LOSS" setelah sempat profit sekarang
// berhenti di BREAKEVEN (~0%), bukan rugi penuh. Trade yang timeout juga
// di-mark-to-market pakai harga close candle terakhir (bukan dipaksa 0%),
// karena posisi yang masih terbuka & profit saat expiry itu nyata, bukan
// wash. Kategori WIN/LOSS/BREAKEVEN/TIMEOUT ini sesuai dok produk 10.6.
function simulateTrade(ticker, tier, direction, entryIdx, candles, entry, stopLossInit, tp1, tp2, scoreAbs, maxHoldBars) {
  const sign = direction === 'BUY' ? 1 : -1;
  const riskPct = Math.abs(entry - stopLossInit) / entry;
  const breakevenTrigger = entry + sign * Math.abs(entry - stopLossInit) // +1R
  ;
  const breakevenStop = entry * (1 + sign * FEE_SLIPPAGE_PCT) // cukup nutup fee, bukan rugi/untung riil
  ;
  let stopCurrent = stopLossInit;
  let reachedBreakeven = false;
  const lastIdx = Math.min(entryIdx + maxHoldBars, candles.length - 1);
  for(let i = entryIdx + 1; i <= lastIdx; i++){
    const bar = candles[i];
    const hitStop = sign === 1 ? bar.low <= stopCurrent : bar.high >= stopCurrent;
    const hitBreakevenTrigger = sign === 1 ? bar.high >= breakevenTrigger : bar.low <= breakevenTrigger;
    const hitTp2 = sign === 1 ? bar.high >= tp2 : bar.low <= tp2;
    if (!reachedBreakeven) {
      if (hitStop) {
        const pnl = sign * (stopCurrent - entry) / entry - FEE_SLIPPAGE_PCT;
        return {
          ticker,
          tier,
          direction,
          entry_ts: candles[entryIdx].ts,
          entry,
          stop_loss: stopLossInit,
          tp1,
          tp2,
          outcome: 'LOSS',
          pnl_pct: pnl,
      ...
      if (hitBreakevenTrigger) {
        ...
        stopCurrent = breakevenStop;
        if (hitTp2) {
          const pnl = sign * (tp2 - entry) / entry - FEE_SLIPPAGE_PCT;
          return {
            ticker,
            tier,
            direction,
  ...
  // Timeout: mark-to-market di close candle terakhir, bukan dipaksa 0%.
  const lastClose = candles[lastIdx].close;
  const pnl = sign * (lastClose - entry) / entry - FEE_SLIPPAGE_PCT;
  return {
    ticker,
    tier,
    direction,
    ...
    pnl_pct: pnl,
    risk_pct: riskPct,
    bars_held: lastIdx - entryIdx,
    score: scoreAbs,
    reached_breakeven: reachedBreakeven
  };
}
function summarize(trades) {
  ...
  // Win Rate hanya dari trade yang benar-benar selesai lewat TP2 atau SL awal
  // (sesuai dok 10.6: WIN/LOSS/BREAKEVEN/INVALID adalah kategori terpisah).
  const winRate = closedTrades > 0 ? wins.length / closedTrades * 100 : 0;
  // Profit Factor dihitung dari SEMUA trade (termasuk breakeven yang nutup
  // fee kecil, dan timeout yang di-mark-to-market) supaya representatif
  // terhadap ekuitas riil sistem, bukan cuma dari trade yang murni WIN/LOSS.
  const allForPnl = [
    ...wins,
    ...losses,
    ...breakevens,
  ...
  const grossProfit = allForPnl.filter((t)=>t.pnl_pct > 0).reduce((sum, t)=>sum + t.pnl_pct, 0);
  const grossLoss = Math.abs(allForPnl.filter((t)=>t.pnl_pct <= 0).reduce((sum, t)=>sum + t.pnl_pct, 0));
...
Deno.serve(async (req)=>{
  ...
  const { data: stocks, error: stErr } = await supabase.from('stocks').select('id, ticker').eq('is_active', true).order('ticker', {
    ascending: true
  }).range(offset, offset + maxStocks - 1);
  if (stErr || !stocks || stocks.length === 0) {
    return new Response(JSON.stringify({
      error: stErr?.message ?? 'tidak ada saham aktif ditemukan'
    }), {
      status: 500
    });
  }
  ...
  for (const stock of stocks){
    const { data: candleRows, error: cErr } = await supabase.from('candles').select('ts, open, high, low, close, volume').eq('stock_id', stock.id).eq('timeframe', timeframe).order('ts', {
      ascending: true
    });
  ...
  for (const stock of stocks){
    ...
    const volAvg20 = new Array(candles.length).fill(null);
    ...
    for(let i = startIdx; i < endIdx; i++){
      ...
      const candle = i >= 2 ? candlestickScore(candles[i - 2], candles[i - 1], candles[i]) : {
        score: 0,
      ...
      const tp2 = entryPrice + 3 * risk;
      const trade = simulateTrade(stock.ticker, tier, direction, entryIdx, candles, entryPrice, stopLoss, tp1, tp2, Math.abs(score), config.maxHoldBars);
  ...
  const tierResults = {};
  const runIds = {};
  for (const tierKey of Object.keys(LIQUIDITY_TIERS)){
    ...
    const { data: inserted, error: insErr } = await supabase.from('backtest_runs').insert({
      ...
      breakevens: summary.breakevens.length,

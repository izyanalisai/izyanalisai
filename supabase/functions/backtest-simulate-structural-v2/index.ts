...
function findPivots(candles) {
  ...
  for(let i = PIVOT_LEFT; i < candles.length - PIVOT_RIGHT; i++){
    ...
    if (candles[i].high === Math.max(...wH)) highs.push({
      idx: i,
      price: candles[i].high
    });
    if (candles[i].low === Math.min(...wL)) lows.push({
      idx: i,
      price: candles[i].low
    });
  }
...
function readStructure(windowCandles) {
  ...
  return {
    bias,
    lastClose,
    supportZones: buildZones(lows, windowCandles.length),
    resistanceZones: buildZones(highs, windowCandles.length)
  };
}
function nearestBelow(zones, price) {
  const b = zones.filter((z)=>z.price_low <= price);
  return b.length ? b.reduce((x, z)=>z.price_high > x.price_high ? z : x) : null;
}
function nearestAbove(zones, price) {
  const a = zones.filter((z)=>z.price_high >= price);
  return a.length ? a.reduce((x, z)=>z.price_low < x.price_low ? z : x) : null;
}
function secondAbove(zones, first) {
  const c = zones.filter((z)=>z.price_low > first.price_high);
  return c.length ? c.reduce((x, z)=>z.price_low < x.price_low ? z : x) : null;
}
function secondBelow(zones, first) {
  const c = zones.filter((z)=>z.price_high < first.price_low);
  return c.length ? c.reduce((x, z)=>z.price_high > x.price_high ? z : x) : null;
}
...
Deno.serve(async (req)=>{
  ...
  if (tier !== 'daily' && tier !== 'swing') return new Response(JSON.stringify({
    error: 'tier must be daily or swing'
  }), {
    status: 400
  });
  ...
  if (stErr || !stocks) return new Response(JSON.stringify({
    error: stErr?.message
  }), {
    status: 500
  });
  ...
  for (const s of stocks){
    const { data: d1rows } = await supabase.from('candles').select('ts, open, high, low, close').eq('stock_id', s.id).eq('timeframe', 'D1').order('ts', {
      ascending: true
    });
    if (!d1rows || d1rows.length < 60) continue;
    let w1rows = [];
    if (tier === 'swing') {
      const { data: w1data } = await supabase.from('candles').select('ts, open, high, low, close').eq('stock_id', s.id).eq('timeframe', 'W1').order('ts', {
        ascending: true
      });
      w1rows = w1data ?? [];
  ...
  function stats(trades) {
    ...
    return {
      ...
      expected_value_r: Number(avgR.toFixed(4)),
      max_drawdown_r: Number(maxDD.toFixed(2)),
      outcome_breakdown: {

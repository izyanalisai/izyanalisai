...
function simulate(direction, entryIdx, candles, entry, stopLoss, tp1) {
  for(let i = entryIdx + 1; i <= Math.min(entryIdx + MAX_HOLD_BARS, candles.length - 1); i++){
    const bar = candles[i];
    if (direction === 'BUY') {
      if (bar.low <= stopLoss) return {
        outcome: 'LOSS',
        pnl: (stopLoss - entry) / entry - FEE_SLIPPAGE_PCT
      };
      if (bar.high >= tp1) return {
        outcome: 'WIN',
        pnl: (tp1 - entry) / entry - FEE_SLIPPAGE_PCT
      };
    } else {
      if (bar.high >= stopLoss) return {
        outcome: 'LOSS',
        pnl: (entry - stopLoss) / entry - FEE_SLIPPAGE_PCT
      };
      if (bar.low <= tp1) return {
        outcome: 'WIN',
        pnl: (entry - tp1) / entry - FEE_SLIPPAGE_PCT
      };
    }
  }
  return {
...
function record(buckets, key, res) {
  if (!buckets[key]) buckets[key] = newBucket();
  const b = buckets[key];
  b.trades++;
  if (res.outcome === 'WIN') {
    b.wins++;
    b.grossProfit += res.pnl;
...
Deno.serve(async (_req)=>{
  ...
  if (stErr || !stocks || stocks.length === 0) {
    return new Response(JSON.stringify({
      error: stErr?.message ?? 'saham LQ45 tidak ditemukan'
    }), {
      status: 500
    });
  }
  ...
  for (const stock of stocks){
    const { data: candleRows, error: cErr } = await supabase.from('candles').select('ts, open, high, low, close, volume').eq('stock_id', stock.id).eq('timeframe', 'D1').order('ts', {
      ascending: true
    });
    ...
    for(let i = START_IDX; i < endIdx; i++){
      ...
      function tryDirection(key, direction) {

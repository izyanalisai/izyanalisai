...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!botToken) {
      ...
      return new Response(JSON.stringify({ skipped: true, reason: 'telegram_bot_token belum diset' }), {
    ...
    if (!user_id || !title) {
      return new Response(JSON.stringify({ error: 'user_id dan title wajib diisi' }), { status: 400, headers: CORS_HEADERS })
    }
    ...
    if (subsErr) {
      return new Response(JSON.stringify({ error: subsErr.message }), { status: 500, headers: CORS_HEADERS })
    }
    ...
    for (const sub of subs ?? []) {
      try {
        const res = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
          ...
          body: JSON.stringify({ chat_id: sub.chat_id, text }),
        })
        const json = await res.json()
        if (json.ok) {
          sent++
          await supabase.from('telegram_subscriptions').update({ last_sent_at: new Date().toISOString(), last_error: null }).eq('id', sub.id)
        } else {
          failed++
          const errDesc = String(json.description ?? 'unknown error')
          await supabase.from('telegram_subscriptions').update({ last_error: errDesc }).eq('id', sub.id)
          if (res.status === 403) {
            await supabase.from('telegram_subscriptions').update({ is_active: false }).eq('id', sub.id)
            deactivated++
          }
        }
      } catch (err) {
    ...
    return new Response(JSON.stringify({ sent, failed, deactivated, total: (subs ?? []).length, reference_id: reference_id ?? null }), {
      status: 200, headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
    })
  } catch (err) {
